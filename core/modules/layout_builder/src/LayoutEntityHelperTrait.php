...
trait LayoutEntityHelperTrait {
   ...
   *   To determine if an entity has an layout override use
  ...
  /**
   * Determines if the original entity used the default storage.
   *
   * @param \Drupal\Core\Entity\EntityInterface $entity
   *   The entity.
   *
  ...
  protected function originalEntityUsesDefaultStorage(EntityInterface $entity) {
    ...
    if ($section_storage instanceof OverridesSectionStorageInterface && !$entity->isNew() && isset($entity->original)) {
      ...
      // If there were no sections in the original entity the storage will
      // revert to the default storage and this is a new override.
