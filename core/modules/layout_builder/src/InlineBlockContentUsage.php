...
class InlineBlockContentUsage {
  ...
  public function getUnused($limit = 100) {
    $query = $this->database->select('inline_block_content_usage', 't');
  ...
  public function removeByLayoutEntity(EntityInterface $entity) {
    $query = $this->database->update('inline_block_content_usage')
      ->fields([
  ...
  public function deleteUsage(array $block_content_ids) {
    $query = $this->database->delete('inline_block_content_usage')->condition('block_content_id', $block_content_ids, 'IN');
    $query->execute();
  }
  ...
  public function getUsage($block_content_id) {
    $query = $this->database->select('inline_block_content_usage');
