...
(($, Drupal, debounce, displace) => {
  ...
  Drupal.offCanvas = {
    /**
     * Generic storage for information about the tray.
     */
    state: {},
    ...
    resetSize(event) {
       ...
       */
      if (
        Drupal.offCanvas.state.hasOwnProperty('position') &&
        Drupal.offCanvas.state.position !== position) {
        container.removeAttr(`data-offset-${Drupal.offCanvas.state.position}`);
      }
      ...
      Drupal.offCanvas.state = { position };
    },
