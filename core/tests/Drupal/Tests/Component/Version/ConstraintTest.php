...
class ConstraintTest extends TestCase {
  ...
  public function providerIsCompatible() {
    ...
    // One or more spaces are supported by not require between commas and
    // version numbers and between operators and version numbers.
    ...
    // ==, =, and no operator are all support for equals.
    foreach ($original_tests as $key => $original_test) {
      foreach (['=', ''] as $equal_operator) {
        if (strpos($original_test[0][0], '==') !== FALSE) {
          $new_key = str_replace('==', $equal_operator, $key);
          $tests[$new_key] = $original_test;
          $tests[$new_key][0][0] = str_replace('==', $equal_operator, $original_test[0][0]);
        }
      }
    }
    return $tests;
